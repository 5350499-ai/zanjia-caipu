...
- 菜谱详情页在“已做 N 次”后显示“首次：YYYY年M月D日”，日期来自最早的 `recipe_cook_events`，不使用菜谱创建时间。
- 详情事件接口与菜谱列表统一返回规范化的首次做菜日期，沿用香港 PostgreSQL DATE 日历日期修复逻辑。

## [做饭次数月度统计一致性] 2026-09-02

- 修复香港 PostgREST 将 PostgreSQL `date` 序列化为带时区时间戳后，月度统计按 UTC 日期前缀分组导致跨日事件落入上月的问题。
- 统一在事件聚合前恢复数据库日历日期，保留家庭统计的 Europe/Madrid 业务语义；新增 9 月 1 日边界回归测试。

## [首页导航数量信息] 2026-09-01

- 恢复登录首页第一排菜谱数量：我的菜谱、全部菜谱/收起菜谱、家庭共享分别显示菜谱集合数量。
- 恢复第二排四位家庭成员的累计实际做饭次数；成员筛选仍按记录人，统计数字仍按 `recipe_cook_events`。

## [登录首页导航与成员快捷入口] 2026-09-01

- 登录用户首页新增“我的菜谱 / 全部菜谱 / 家庭共享 / 收藏”四项主导航；“全部菜谱”即时展开/收起当前用户完整菜谱。
- 家庭成员快捷导航对登录用户与游客统一可用，成员筛选继续按菜谱记录人（`author_user_id`）读取，并支持完整列表、趋势和 Top20 视图。
- 点击导航先同步更新视图状态，避免等待网络或图片加载造成无响应感；未修改数据库、认证、图片或统计数据。
