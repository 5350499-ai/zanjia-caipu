...
async function requestDirect(path, options = {}) {
  ...
  if (method === 'POST') {
    ...
    rows.forEach(row => { tuples.push(`(${columns.map(col => { const key = col.slice(1, -1); const value = row[key] ?? null; bind.push(value && typeof value === 'object' ? JSON.stringify(value) : value); return `$${bind.length}` }).join(',')})`) })
  ...
  if (method === 'PATCH') {
    const entries = Object.entries(rows[0] || {}); const sets = entries.map(([key, value]) => { values.push(value && typeof value === 'object' ? JSON.stringify(value) : value); return `${ident(key)} = $${values.length}` })
