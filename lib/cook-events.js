...
// The HK PostgREST adapter serializes PostgreSQL DATE values as midnight in
// its Asia/Shanghai session timezone (for example, 2026-09-01 becomes
// 2026-08-31T16:00:00.000Z). Recover the stored calendar date before applying
// the family's Europe/Madrid reporting rules.
const DATABASE_DATE_TIME_ZONE = 'Asia/Shanghai'
...
async function countRecipeEvents(recipeId) {
  ...
  const dates = rows.map(row => calendarDate(row.cooked_on)).filter(Boolean).sort()
  return {
    count: rows.length,
    firstCookedOn: dates[0] || null,
    lastCookedAt: dates[dates.length - 1] || null,
  }
}
...
function calendarDate(value) {
  if (value instanceof Date) return new Intl.DateTimeFormat('en-CA', { timeZone: DATABASE_DATE_TIME_ZONE, year: 'numeric', month: '2-digit', day: '2-digit' }).format(value)
  const text = String(value || '')
  if (/^\d{4}-\d{2}-\d{2}T/.test(text)) {
    return new Intl.DateTimeFormat('en-CA', { timeZone: DATABASE_DATE_TIME_ZONE, year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date(text))
  }
...
function inPeriod(dateValue, period = 'all', year, month) {
  ...
  const value = calendarDate(dateValue)
...
function buildAnnualCookingTrend(members, recipes, events, { year, currentYear, currentMonth } = {}) {
  ...
  events.forEach(event => {
    ...
    const dateMatch = cookedOn.match(/^(\d{4})-(\d{2})-(\d{2})$/)
    if (!dateMatch || Number(dateMatch[1]) !== targetYear) return
...
module.exports = { DAILY_COOK_SOURCES, FAMILY_TIME_ZONE, madridDate, canViewRecipe, listFamilyEvents, countRecipeEvents, ensureFirstCookEventForImageRecipe, ensureImageBaseline, buildMonthlyRanking, buildRanking, buildFamilyStats, buildAnnualCookingTrend, calendarDate }
